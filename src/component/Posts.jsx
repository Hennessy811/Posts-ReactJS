...
export default function Posts({ result }) {
  ...
  // тут выносить необязательно)
  function handlerChangeInput(e) {
    setvalue(e.target.value);
  }

  // саму функцию можно вынести наружу
  function filtersPosts(arr, value) {
    if (value === '') {
      return arr;
    } else {
      const resultFilters = arr.filter(el => {
        // можно вынести в небольшую функцию, чтобы не копипастить
        ...
        // FIXME: filter должен вернуть true/false для *каждого* элемента массива
  ...
  // было бы хорошо мемоизировать эту функцию, чтобы не пересчитывать на каждый рендер
